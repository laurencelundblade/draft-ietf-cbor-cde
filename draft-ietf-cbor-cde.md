...
The payload’s primary purpose is diagnostic information to explain why a NaN was generated by a local computation.
...
The only way to work with a NaN payload is to cast the floating-point value to an integer and then use shifts and masks to unpack the IEEE-754 representation.
...
A CBOR implementation that doesn’t do preferred serialization of floats will usually just pass a NaN, including a NaN with a payload through as part of its floating-point implementation.
It’s very simple to implement CBOR floating-point encoding/decoding without preferred serialization.

But most CBOR implementations should support preferred serialization of floats.
This implies preferred serialization of NaN payloads.
Specifically, if a NaN payload has trailing zero bits that can be dropped to fit the NaN with payload into a smaller type of float, then this must be done.

For example, take a double NaN with a payload, shown here in its encoded form: 0x7ffFFFFFE0000000.
It unpacks to the sign bit as clear, the exponent bits 0x7ff and a 52-bit payload with the 23 leftmost bits set and 29 rightmost bits clear.
This must be encoded as a 0x7fffffff, a single-precision float, because all 23 payload bits fit.
If the payload had 24 bits set, they would not and it must be left as double-precision.

The algorithm for this conversion is essentially the same as for preferred serialization of non-NaN values.
