...
CBOR libraries that support floating-point must support NaNs and may consider supporting NaN payloads.
...
Its original purpose is to represent things like division by zero when doing local calculation.
CBOR allows NaN to be transmitted in a protocol for use as an out-of-band protocol indicator.
NaN is widely supported in IEEE-754 hardware and software by CPUs, OS’s and programming environments.
CBOR protocol designs can generally rely on NaN being supported.

Note that jSON doesn't allow NaN.
For protocols that are to work in both CBOR and JSON and need an out-of-band indicator, consider this (in CDDL):

    Float-with-oob = float / NULL

### NaN Payloads

An IEEE-754 NaN has up to 52 extra bits called the “payload”.
The payload’s original purpose is diagnostic information to explain why a NaN was generated by a local computation.
...
If a protocol design prohibits NaN payloads and is using preferred serialization then NaN must be encoded as a half-precision with the payload as 0, specifically 0xF97E00.
...
NaN payloads have been in the IEEE-754 standard since 2008, but programming environments do not provide facilities (e.g., APIs) to make use of them.
For example, in C there is the isnan() API to check if a value is a NaN, but there are no APIs to construct or access the NaN payload.
The typical way to work with a NaN payload is to cast the floating-point value to an integer and then use shifts and masks to unpack the IEEE-754 representation.
Python, Java and such are similar.

In the CBOR world, there is no support in CDDL or EDN for NaN payloads.

As a CBOR protocol designer, better constructs are available than the NaN payload.
...
It is supported in EDN and CDDL.
It is easy implementable every programming environment.
...
In all the above CDDL constructs, float never is a NaN.
...
There are very few MUSTs in CBOR so as to allow its use in a large variety of constrained use cases.
For example, support for integers is not required because a protocol might need only strings.
There is no MUST that requires support of NaN and NaN payloads, but the recommendation here is that any CBOR library that supports floating-point should support NaNs without payloads.
CBOR libraries may consider supporting NaN payloads.

In most environments, there is little extra work to do to support NaN without payloads if floating-point is supported.
NaNs will usually flow through as any other floating-point value.

CBOR libraries should support preferred serialization of floating-point including NaNs.
For NaNs without payloads that requires reducing to a half-precision NaN without a payload.
This requires a few explicit extra lines of code.
See the sample half-precision implementation in Appendix D of RFC 8949.

The implementation of preferred serialization of NaN payloads needs more care.
As with preferred serialization, NaN payloads must be reduced but only if they can be reduced without the loss of any non-zero payload bits.
Programming platform provided floating-point hardware and software may or may not do this correctly for double to single conversion.
The sample half-precision implementation in Appendix D of RFC 8949 only supports NaNs without payloads.

A double NaN payload contains 52 bits, a single 23 bits and a half 10 bits.
A double NaN can be reduced to a single only if the right-most 29 payload bits are zero.
A single NaN can be reduced to a half only if the right-most 13 payload bits are zero.
And of course a double NaN can be reduced to half only if the right-most 42 payload bits are zero.
This is the same reduction as regular, non-NAN, floating-point values, but the implementations for non-NaN values may or may not address NaN payloads.

To implement the above, most CBOR libraries will have to convert the float to an integer and use shifts and masks.
This will require knowledge of the internal representation defined in IEEE-754.

Testing on some CPUs has shown them to do this correctly for conversion between single and double.
However, it may not be useful to rely on platform libraries for the following reasons.
First, they may provide no support at all for half-precision and half-precision is required for preferred serialization.
Second, NaN payloads are a relatively recent and very uncommonly used feature whos design purpose is local calculation diagnostics.

If platform implementation is relied upon, NaN payload reduction should be tested on each platform.
Open source libraries intended to run on multiple platforms may be better not relying on the platform.
...
| 0x7ff80000000003ff | 0xfb7ff80000000003ff | 10-bit payload that can't be reduced |
...
| 0x7fbff000 | 0xfa7fbff000 | single 10-bit payload that can't be reduced |
